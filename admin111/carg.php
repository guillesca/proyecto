...
<hr  width=100% size=10 color="#0B173B">
 <center>
  ...
  <form action="" method='post' enctype="multipart/form-data">
      <input type="date" name="fecha_ve" style="border-radius: 30px;width: 190px"  class="form-control"   placeholder="Fecha de vencimiento" required/>  
      <br>
        ...
        //Aquí es donde seleccionamos nuestro csv
         ...
         if(strtolower(end($chk_ext)) == "csv")
         {
             ...
             while (($data = fgetcsv($handle, 1000, ",")) !== FALSE)
             {
               ...
               $sql2= "INSERT INTO alta (id,super,usua,fecha) VALUES ('','".$_SESSION['empleado']."','$data[1]','$actual')";
             mysql_query($sql2) or die('Error: '.mysql_error());

                $sql = "INSERT into empleado2 (id,empleado,nombre,pass,fecha_at,fecha_ve,super) values('','$data[1]','$data[2]','$data[3]','$actual','".$_POST["fecha_ve"]."','".$_SESSION['empleado']."')";
                mysql_query($sql) or die('Error: '.mysql_error());
             }
             //cerramos la lectura del archivo "abrir archivo" con un "cerrar 
